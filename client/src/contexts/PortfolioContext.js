...
export const PortfolioProvider = ({ children }) => {
  ...
  const fetchPortfolio = async () => {
    ...
    if (!token) return;
    const response = await fetch('https://crytosim1.onrender.com/api/portfolio', {
  ...
  const buyCrypto = async (crypto, amount) => {
    const token = localStorage.getItem('token');
    const response = await fetch('https://crytosim1.onrender.com/api/portfolio/buy', {
      method: 'POST',
      headers: {
  ...
  const sellCrypto = async (crypto, amount) => {
    const token = localStorage.getItem('token');
    const response = await fetch('https://crytosim1.onrender.com/api/portfolio/sell', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
